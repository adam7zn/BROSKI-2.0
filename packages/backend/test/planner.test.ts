...
import { reviewStateFor, reviewStates, type AttemptRecord } from '../src/review.js';
import type { StudyItem } from '../src/study-plan.js';

const items: StudyItem[] = [
  { id: 'balance', topic: 'balans', sourceText: 'a', difficulty: 'easy', image: null },
  { id: 'parens', topic: 'parenteser', sourceText: 'b', difficulty: 'medium', image: null },
  { id: 'quadratic', topic: 'andragrad', sourceText: 'c', difficulty: 'medium', image: null },
];
...
test('lesson times follow the course timezone across a DST change', () => {
  const summer = lessonStart(
    { id: 's', startsAt: '2026-07-01 09:15', topic: '', covers: [], prepares: [] },
    'Europe/Stockholm',
  );
  const winter = lessonStart(
    { id: 'w', startsAt: '2026-12-01 09:15', topic: '', covers: [], prepares: [] },
    'Europe/Stockholm',
  );
...
test('the interval is capped so nothing disappears for good', () => {
  const many = Array.from({ length: 12 }, (_, index) =>
    attempt('balance', 'correct', `2026-09-${String(index + 1).padStart(2, '0')}T10:00:00Z`),
  );
