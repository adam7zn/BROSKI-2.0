...
import { backendContextSchema, type BackendContext } from '@msc/conversation';
...
export function selectStudyItem(
  ...
  const sorted = [...items].sort(
    (a, b) =>
      Date.parse(lastUsed.get(a.id)!) - Date.parse(lastUsed.get(b.id)!),
  );
  const item = sorted[0]!;
...
export function toBackendContext(item: StudyItem): BackendContext {
