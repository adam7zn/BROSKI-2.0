...
export function planStudySession(input: PlanInput): PlanDecision {
  ...
  if (previous && previous.hoursAway <= practiseWindowHours) {
    const item = weakestFirst(resolve(previous.lesson.covers, byId), reviews)[0];
    if (item) {
      return {
        mode: 'PRACTISE',
        item,
  ...
  const due = items
    ...
    .filter(({ state }) => state === null || state.dueAt.getTime() <= now.getTime())
...
function describeQuiet(
  ...
  if (soonest) {
    parts.push(`next review in ${describeDays(daysBetween(now, soonest.dueAt))}`);
  }
  if (upcoming) {
