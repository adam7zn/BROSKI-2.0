...
export class PostgresSourceContentRepository {
    ...
    try {
        ...
        const pageResult = await client.query<{ id: string }>(
          ...
          [
            ...
            JSON.stringify({
              ...
              structuredReviewStatus: 'pending',
            }),
            runId,
          ],
        );
        ...
        for (const block of page.blocks) {
          ...
          const preferred =
            block.candidates.find(
              (candidate) => candidate.passName === 'original',
            ) ?? block.candidates[0]!;
          const blockResult = await client.query<{ id: string }>(
            ...
            [
              ...
              block.confidence,
              JSON.stringify(block.reviewReasons),
              preferred.contentMarkdown,
            ],
          );
      ...
      await client.query(
        `UPDATE source_extraction_runs SET status = 'completed',
           completed_at = COALESCE(completed_at, CURRENT_TIMESTAMP) WHERE id = $1`,
        [runId],
      );
  ...
  async getPage(pageId: string): Promise<SourcePageDetail> {
    ...
    return {
      ...toPageSummary(page),
      width:
        typeof page.extraction_metadata.width === 'number'
          ? page.extraction_metadata.width
          : null,
      height:
        typeof page.extraction_metadata.height === 'number'
          ? page.extraction_metadata.height
          : null,
  ...
  async getBlockImageReference(
    blockId: string,
  ): Promise<SourceBlockImageReference> {
    const result = await this.pool.query<
      BlockRow & { page_image_key: string | null }
    >(
      `SELECT b.id, b.source_page_id, b.sequence_number, b.block_type, b.bounding_box,
              b.confidence, b.review_state, b.review_reasons, b.current_content_markdown,
              p.page_image_key
  ...
  async createBlock(
    ...
    const found = await this.pool.query<{ extraction_run_id: string }>(
      `SELECT active_extraction_run_id AS extraction_run_id FROM source_pages
       WHERE id = $1 AND active_extraction_run_id IS NOT NULL`,
      [pageId],
    );
    const runId = found.rows[0]?.extraction_run_id;
    if (runId === undefined)
      throw new Error(`Page ${pageId} has no extraction run.`);
    ...
    try {
      ...
      const accepted = await client.query<{ current_content_markdown: string }>(
        `SELECT current_content_markdown FROM source_blocks
         WHERE source_page_id = $1 AND deleted_at IS NULL AND block_type <> 'footer'
           AND extraction_run_id = (SELECT active_extraction_run_id FROM source_pages WHERE id = $1)
           AND review_state = 'approved' ORDER BY sequence_number, id`,
        [pageId],
      );
