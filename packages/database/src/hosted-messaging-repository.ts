...
export class PostgresHostedMessagingRepository {
  ...
  async history(interactionId: string): Promise<
    Array<{ direction: 'inbound' | 'outbound'; text: string; occurredAt: string }>
  > {
      ...
      await client.query(
        `UPDATE demo_outbound_outbox
         SET delivery_status = CASE
             ...
             END,
             failure_code = CASE
               WHEN $2 = 'failed' THEN 'SENDBLUE_ERROR'
               ELSE failure_code
             END,
             updated_at = $3
         WHERE provider_message_id = $1`,
        [input.providerMessageId, status, input.now],
      );
      if (status === 'failed') {
        await client.query(
          `UPDATE demo_messaging_sessions
           SET status = 'failed', failure_code = 'SENDBLUE_ERROR', updated_at = $2
           WHERE interaction_id = $1`,
          [message.interaction_id, input.now],
        );
      }
      await client.query('COMMIT');
      return 'recorded';
