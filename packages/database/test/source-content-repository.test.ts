...
import { PostgresSourceContentRepository, runMigrations } from '../src/index.js';

const connectionString = process.env.TEST_DATABASE_URL ?? process.env.DATABASE_URL
  ?? 'postgresql://postgres@127.0.0.1:54329/math_study_companion';
const extraction: StructuredExtraction = {
  schemaVersion: 1, pipelineVersion: 'test-v1', documentTitle: 'Test chapter',
  inputChecksum: createHash('sha256').update('structured-test').digest('hex'),
  configuration: { externalRequests: false },
  pages: [{
    filePageNumber: 1, printedPageNumber: '9', imagePath: '/tmp/page-009.jpg',
    width: 1000, height: 1500,
    blocks: [{
      sourceKey: 'page-1-block-1', sequenceNumber: 1, blockType: 'prose',
      boundingBox: [.1, .1, .8, .2], confidence: .95, reviewReasons: [],
      candidates: [{ engine: 'apple_vision', passName: 'original',
        contentMarkdown: 'Algebraiska uttryck', latex: null, confidence: .95, metadata: {} }],
    }],
  }],
};
...
describe.skipIf(process.env.TEST_DATABASE_URL === undefined)(
  ...
  it('cannot finalize pending content and assembles only approved blocks', async () => {
    ...
    await expect(repository.finalizePage(page.id)).rejects.toThrow('Every meaningful block');
    await repository.reviewBlock(detail.blocks[0]!.id, {
      decision: 'correct', contentMarkdown: 'Korrigerad åäö', reviewer: 'test', notes: null,
    });
    await expect(repository.finalizePage(page.id)).resolves.toBe('Korrigerad åäö');
    await expect(repository.getPage(page.id)).resolves.toMatchObject({ verifiedAt: expect.any(Date) });
  });

  it('keeps candidate and review evidence append-only', async () => {
    const imported = await repository.importExtraction(extraction);
    const page = await repository.getPage((await repository.listPages(imported.documentId))[0]!.id);
    await repository.reviewBlock(page.blocks[0]!.id, {
      decision: 'approve', contentMarkdown: 'Algebraiska uttryck', reviewer: 'test', notes: null,
    });
    await expect(pool.query(`UPDATE source_block_candidates SET content_markdown = 'changed'`))
      .rejects.toThrow('append-only');
    await expect(pool.query(`DELETE FROM source_block_reviews`)).rejects.toThrow('append-only');
  });
  },
);
