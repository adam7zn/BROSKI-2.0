...
test('supports only HTTPS media URLs and does not fetch when disabled', async () => {
  ...
  await provider.sendImage({
    conversationId: '+46700000000',
    media: { kind: 'url', url: 'https://cdn.example.test/example.png' },
    altText: 'equation',
    caption: 'Solve this.',
  ...
  assert.throws(
    () =>
      provider.sendImage({
        conversationId: '+46700000000',
        media: { kind: 'file', path: '/tmp/private.png' },
        altText: 'private',
  ...
  await assert.rejects(
    send(
      providerWith(async () =>
        jsonResponse({
          status: 'QUEUED',
          message_handle: 'invalid-service-1',
          service: 42,
        }),
      ),
    ),
...
test('fails closed when the iMessage lookup response is ambiguous', async () => {
  const provider = new SendblueMessagingProvider({
    ...baseOptions,
    fetchImplementation: (async () =>
      jsonResponse({
...
function inboundWebhook(): Record<string, unknown> {
  return {
    content: 'x = 4',
    is_outbound: false,
    status: 'RECEIVED',
    message_handle: 'incoming-strict-1',
    date_sent: '2026-08-29T12:00:01.000Z',
    from_number: '+46700000000',
    to_number: '+13470000000',
    sendblue_number: '+13470000000',
