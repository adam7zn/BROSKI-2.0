...
const CONFIRMATION = "Thanks — I've saved your setup. Here's one quick warm-up.";
  ...
  async handleInbound(
    ...
    if (state.step === 'level') {
      ...
      return output({
        ...
        agentState: { step: 'grade', course: state.course, level },
        profile: null,
        result: null,
        status: 'waiting',
      });
...
function parseState(value: unknown): DemoAgentState {
  ...
  if (!['course', 'level', 'grade', 'answer', 'complete'].includes(value.step)) {
