...
export interface SendblueServiceAvailability {
  service: 'iMessage';
  checkedAt: string;
}
...
export class SendblueMessagingProvider implements MessagingProvider {
  ...
  sendImage(input: OutboundImage): Promise<SendResult> {
    ...
    if (input.media.kind !== 'url') {
      throw new SendblueError(
        'rejected',
        false,
        'Sendblue media must be an HTTPS URL',
      );
    }
    let mediaUrl: URL;
    try {
      mediaUrl = new URL(input.media.url);
    } catch (error) {
      throw new SendblueError(
        'rejected',
        false,
  ...
  async checkServiceAvailability(): Promise<SendblueServiceAvailability> {
    await this.#assertIMessageCapable();
    return { service: 'iMessage', checkedAt: this.#now().toISOString() };
  }
  ...
  async #send(content: Record<string, string>): Promise<SendResult> {
    ...
    if (
      ...
      (body.opted_out !== undefined &&
        body.opted_out !== null &&
        typeof body.opted_out !== 'boolean') ||
    ...
    if (body.opted_out === true) {
      throw new SendblueError(
        'opt_out',
        false,
        'Sendblue reports that the recipient opted out',
      );
    }
    if (
      typeof body.service === 'string' &&
...
export function normalizeSendblueWebhook(
  ...
  if (payload.is_outbound) {
    const downgraded = optionalBoolean(
      payload.was_downgraded,
      'was_downgraded',
    );
    const optedOut = optionalBoolean(payload.opted_out, 'opted_out');
    const service = requiredString(payload.service, 'service');
    const eventType =
      status === 'SENT'
        ? 'sent'
        : status === 'DELIVERED'
    ...
    const transportFailed =
      downgraded === true ||
      optedOut === true ||
      service.toLowerCase() !== 'imessage';
    if (!eventType && !transportFailed) {
      return { kind: 'ignored', reason: 'non-terminal-status' };
    }
    return {
      kind: 'delivery',
      ...
      eventType: transportFailed ? 'failed' : eventType!,
      occurredAt,
  ...
  const optedOut = optionalBoolean(payload.opted_out, 'opted_out');
  const downgraded = optionalBoolean(payload.was_downgraded, 'was_downgraded');
  const service = strictOptionalString(payload.service, 'service') ?? '';
  return {
    kind: 'inbound',
...
function strictOptionalString(value: unknown, name: string): string | null {
