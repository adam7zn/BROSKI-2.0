...
const mathPattern = /[=≈≠≤≥±√∫∑∞^_]|\b(?:lim|sin|cos|tan|log)\b|\d\s*[+−*/]\s*\d/iu;
...
export const classifyBlock = (block: RawVisionBlock): SourceBlockType => {
  ...
  if (block.boundingBox[1] > 0.94 || (/^\d{1,3}$/.test(text) && block.boundingBox[1] > 0.88)) {
    return 'footer';
  }
  if (mathPattern.test(text) && (text.length < 140 || text.split('\n').length <= 3)) {
    return 'formula';
  }
...
const normalized = (value: string): string =>
  value.normalize('NFKC').replace(/\s+/gu, ' ').trim().toLocaleLowerCase('sv-SE');
...
export const validateLatex = (latex: string): string[] => {
  ...
  for (const [opening, closing] of pairs) {
    ...
    for (const character of latex) {
      if (character === opening && (opening !== '{' || !latex.includes(`\\${opening}`))) depth += 1;
      if (character === closing && (closing !== '}' || !latex.includes(`\\${closing}`))) depth -= 1;
      if (depth < 0) break;
    }
    if (depth !== 0) flags.push('unbalanced_brackets');
  }
  if (/\ufffd|�/u.test(latex)) flags.push('suspicious_character');
  if (/\\(?:frac|sqrt)\b(?!\s*\{)/u.test(latex)) flags.push('malformed_latex_command');
...
export const removeLayoutDuplicates = (blocks: RawVisionBlock[]): RawVisionBlock[] =>
  blocks.filter((block, index) => !blocks.some((other, otherIndex) =>
    otherIndex !== index && isSpecialized(other) && !isSpecialized(block)
...
export const groupTextBlocks = (blocks: RawVisionBlock[]): RawVisionBlock[] => {
  ...
  for (const block of ordered) {
    ...
    if (previous === undefined || previous.kind !== 'paragraph' || block.kind !== 'paragraph') {
      grouped.push(block);
      continue;
    }
    ...
    const aligned = Math.abs(previousBox[0] - currentBox[0]) < 0.08 || overlapRatio > 0.3;
    if (gap < -0.004 || gap > 0.018 || previousColumn !== currentColumn || !aligned) {
      grouped.push(block);
      continue;
    }
    const left = Math.min(previousBox[0], currentBox[0]);
    const top = Math.min(previousBox[1], currentBox[1]);
    const right = Math.max(previousBox[0] + previousBox[2], currentBox[0] + currentBox[2]);
    const bottom = Math.max(previousBox[1] + previousBox[3], currentBox[1] + currentBox[3]);
...
export const orderBlocks = (blocks: RawVisionBlock[]): RawVisionBlock[] => {
  ...
  const sectionFor = (block: RawVisionBlock): number =>
    fullWidth.filter((separator) => separator.boundingBox[1] < block.boundingBox[1]).length;
  return [...blocks].sort((a, b) => {
    ...
    return a.boundingBox[1] - b.boundingBox[1] || a.boundingBox[0] - b.boundingBox[0];
  });
};

const sourceKey = (page: number, block: RawVisionBlock): string =>
  createHash('sha1').update(`${page}:${block.kind}:${block.boundingBox.join(',')}:${normalized(block.transcript)}`)
    .digest('hex');
...
export const structurePage = (page: RawVisionPage): ExtractedSourceBlock[] => {
  ...
  if (originalPass === undefined) throw new Error(`Page ${page.filePageNumber} has no original pass.`);
  const originals = groupTextBlocks(removeLayoutDuplicates(originalPass.blocks));
  const contrasts = groupTextBlocks(removeLayoutDuplicates(contrastPass?.blocks ?? []));
  ...
  return originals.map((block, index) => {
    ...
    for (const contrastIndex of unusedContrast) {
      const overlap = intersectionOverUnion(block.boundingBox, contrasts[contrastIndex]!.boundingBox);
      if (overlap > bestOverlap) { bestOverlap = overlap; bestIndex = contrastIndex; }
    }
    const match = bestIndex !== undefined && bestOverlap >= 0.2
      ? contrasts[bestIndex] : undefined;
    if (bestIndex !== undefined && match !== undefined) unusedContrast.delete(bestIndex);
    const candidates: SourceCandidate[] = [{
      engine: 'apple_vision', passName: 'original', contentMarkdown: block.transcript,
      latex: null, confidence: block.confidence,
      metadata: { kind: block.kind, perspectiveCorrected: page.perspectiveCorrected },
    }];
    if (match !== undefined) candidates.push({
      engine: 'apple_vision', passName: 'contrast', contentMarkdown: match.transcript,
      latex: null, confidence: match.confidence, metadata: { kind: match.kind, overlap: bestOverlap },
    });
    const type = classifyBlock(block);
    const agreement = match === undefined ? 0 : candidateAgreement(block.transcript, match.transcript);
    ...
    if (/\ufffd|�|[|]{3,}/u.test(block.transcript)) reviewReasons.push('suspicious_character');
    if (type === 'formula') reviewReasons.push('math_requires_review');
    if (type === 'graph' || type === 'image') reviewReasons.push('visual_block_requires_review');
    return {
      sourceKey: sourceKey(page.filePageNumber, block),
      sequenceNumber: index + 1,
      blockType: type,
      boundingBox: block.boundingBox,
      confidence: match === undefined ? block.confidence : Math.min(block.confidence, match.confidence),
