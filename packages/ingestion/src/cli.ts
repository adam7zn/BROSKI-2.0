...
import { structuredExtractionSchema, type ExtractedSourceBlock } from '@math-study-companion/contracts';
...
const filenames = (await readdir(checkpoints))
  .filter((name) => /^page-\d+\.json$/u.test(name)).sort();
if (filenames.length === 0) throw new Error(`No page checkpoints found in ${checkpoints}.`);
const pages = await Promise.all(filenames.map(async (name) =>
  JSON.parse(await readFile(path.join(checkpoints, name), 'utf8')) as RawVisionPage));
...
if (process.argv.includes('--pix2tex')) {
  ...
  const python = process.env.BROSKI_OCR_PYTHON
...
try {
  for (const page of pages) {
    ...
    if (client !== undefined) {
      for (const block of blocks.filter((item) => item.blockType === 'formula')) {
        try {
          const result = await client.recognize(page.imagePath, block.boundingBox);
          if (result.latex !== undefined && result.latex.trim() !== '') {
            const latex = result.latex.trim();
            block.candidates.push({
              engine: 'pix2tex', passName: 'formula',
              contentMarkdown: `$$\n${latex}\n$$`, latex,
              confidence: result.confidence ?? null, metadata: { device: 'cpu', batchSize: 1 },
            });
          ...
          block.candidates.push({
            engine: 'pix2tex', passName: 'formula-error', contentMarkdown: '', latex: null,
            confidence: null, metadata: { error: error instanceof Error ? error.message : String(error) },
          });
        }
    ...
    extractedPages.push({
      ...
      imagePath: path.resolve(page.imagePath), width: page.width, height: page.height,
      blocks: blocks as ExtractedSourceBlock[],
    });
    console.info(`structured page ${page.printedPageNumber ?? page.filePageNumber}`);
  }
} finally {
  await client?.close();
}
...
const extraction = structuredExtractionSchema.parse({
  ...
  configuration: {
    ...
    pix2texDevice: 'cpu', batchSize: 1,
  },
  pages: extractedPages,
});
await writeFile(path.resolve(outputValue), `${JSON.stringify(extraction, null, 2)}\n`, 'utf8');
console.info(`wrote ${extraction.pages.length} pages to ${path.resolve(outputValue)}`);
