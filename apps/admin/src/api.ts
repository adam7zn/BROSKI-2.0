import type { BoundingBox, BlockType, DocumentSummary, PageDetail, PageSummary } from './types';
...
export const api = {
  documents: (token: string) => request<DocumentSummary[]>(token, '/internal/content/documents'),
  pages: (token: string, documentId: string) =>
    request<PageSummary[]>(token, `/internal/content/documents/${documentId}/pages`),
  ...
  layout: (token: string, blockId: string, value: {
    sequenceNumber: number; blockType: BlockType; boundingBox: BoundingBox;
  }) => request(token, `/internal/content/blocks/${blockId}/layout`, {
    method: 'PATCH', body: JSON.stringify(value),
  }),
  review: (token: string, blockId: string, value: {
    decision: 'approve' | 'correct' | 'reject'; contentMarkdown: string;
  }) => request(token, `/internal/content/blocks/${blockId}/reviews`, {
    method: 'POST', body: JSON.stringify({ ...value, reviewer: 'william', notes: null }),
  }),
  ...
  finalize: (token: string, pageId: string) =>
    request(token, `/internal/content/pages/${pageId}/finalize`, { method: 'POST' }),
};
