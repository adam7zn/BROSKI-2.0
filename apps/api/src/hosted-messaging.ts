...
export class HostedMessagingService {
  ...
  async ingestWebhook(
    ...
    return outcome === 'not_found'
      ? { outcome: 'ignored', reason: 'no-active-session' }
      : { outcome: outcome === 'queued' ? 'queued' : 'duplicate' };
  }
  ...
  async inspect(interactionId: string, traceId: string) {
    ...
    const sessionMetadata = session
      ? (({ agentState: _agentState, ...metadata }) => metadata)(session)
      : null;
    return {
      session: sessionMetadata,
      inbound: inbound.map(({ content: _content, ...metadata }) => metadata),
      outbox: outbox.map(({ content: _content, ...metadata }) => metadata),
    };
  }
...
function validateAgentOutput(
  ...
  throw agentOutputError(traceId, 'Conversation agent output failed validation');
}
