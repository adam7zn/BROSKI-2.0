...
export class InMemoryHostedMessagingRepository implements HostedMessagingRepository {
  ...
  async completeInbound(input: {
    ...
    if (input.output.result) {
      const resultOutcome = await this.interactions.saveResult(
      ...
      if (resultOutcome !== 'saved') {
        ...
        this.#sessions.set(session.interactionId, {
          ...session,
          status: 'failed',
          failureCode: 'INTERACTION_ALREADY_COMPLETED',
          updatedAt: input.now,
        });
        return 'stale';
      }
  ...
  async recordDelivery(input: {
    ...
    if (input.event.eventType === 'failed') {
      await this.stopSession({
        interactionId: message.interactionId,
        status: 'failed',
        failureCode: input.failureCode ?? 'SENDBLUE_ERROR',
        now: input.now,
      });
    }
    return 'recorded';
...
function inboundKey(
  value: Pick<InboundMessage, 'provider' | 'providerEventId'>,
) {
