...
import {
  InMemoryHostedMessagingRepository,
...
test('runs launch -> Sendblue webhook replies -> agent -> persisted result', async () => {
  ...
  assert.equal((await fixture.reply('Mathematics 3c', 'in-1')).outcome, 'queued');
  assert.match(fixture.provider.sent[1]?.text ?? '', /struggling, okay, or confident/);
...
test('deduplicates inbound handles and filters wrong, group, and old events', async () => {
  ...
  const group = await fixture.app.messaging!.ingestWebhook(
    { ...inboundPayload('group', 'group-1', fixture.nextTime()), group_id: 'g' },
    webhookSecret,
    'trace-webhook',
  );
  ...
  assert.equal((await fixture.repository.findSession('demo-001'))?.status, 'active');
...
test('fails closed on opt-out and SMS downgrade webhooks', async () => {
  ...
  await optedOut.app.messaging!.ingestWebhook(
    { ...inboundPayload('STOP', 'stop-1', optedOut.nextTime()), opted_out: true },
    webhookSecret,
    'trace-webhook',
  );
  assert.equal(
...
test('requires bearer auth for internal routes and separate webhook auth', async () => {
  ...
  const invalidWebhook = await fetch(
    `${baseUrl}/webhooks/messaging/sendblue`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify(inboundPayload('secret body', 'bad-1', new Date().toISOString())),
    },
  );
  assert.equal(invalidWebhook.status, 401);
  assert.equal((await invalidWebhook.json()).code, 'INVALID_WEBHOOK_AUTHENTICATION');
});
...
test('reserves before provider invocation and never resends a duplicate reservation', async () => {
  ...
  assert.equal((await fixture.repository.listOutbox('demo-001'))[0]?.deliveryStatus, 'uncertain');
});
